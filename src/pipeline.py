...
class PipelineOrchestrator:
    ...
    def _run_news_crawler(self) -> bool:
        ...
        try:
            ...
            crawler = NewsCrawler()
            Config.update_dates(days_back=29)
    ...
    def _run_summarizer(self) -> bool:
        ...
        try:
            ...
            topic_files = list(Config.TOPICS_DIR.glob("*.csv"))

            if not topic_files:
                display_error(
                        "No topic CSV files found. Please run topic clustering first."
                )
                return False
        ...
        with ProgressManager("Pipeline Execution", len(filtered_steps)) as progress:
          for i, (step_id, step_name, step_func) in enumerate(filtered_steps):
            ...
            if success:
                ...
                progress.update(1, f"Failed at {step_name}")
